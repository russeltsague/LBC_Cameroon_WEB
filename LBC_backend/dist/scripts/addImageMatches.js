...
const normalizeCategory = (cat) => {
    ...
    if (cat.includes('U18 FILLES'))
        return 'U18 FILLES';
    if (cat.includes('U18 GARCONS'))
        return 'U18 GARCONS';
    if (cat.includes('L2A'))
        return 'L2A MESSIEUR';
    if (cat.includes('L2B'))
        return 'L2B MESSIEUR';
    if (cat.includes('L1 MESSIEURS'))
        return 'L1 MESSIEUR';
    if (cat.includes('L1 MESSIEUR'))
        return 'L1 MESSIEUR';
    if (cat.includes('DAMES'))
        return 'L1 DAME';
    if (cat.includes('MESSIEURS'))
        return 'L1 MESSIEUR';
    return cat;
};
// Helper to find closest team name in the same category, allowing for suffixes for L1 MESSIEUR and L1 DAME
function findClosestTeamInCategory(name, category, teams) {
    // Normalize category
    ...
    for (const t of candidates) {
        ...
        if (cat === 'L1 MESSIEUR' && (t.name.toUpperCase().replace(/ MESSIEURS?$/, '') === name.toUpperCase()))
            return t.name;
        if (cat === 'L1 DAME' && (t.name.toUpperCase().replace(/ DAMES?$/, '') === name.toUpperCase()))
            return t.name;
        // Fuzzy match
        const dist = fast_levenshtein_1.default.get(name.toUpperCase(), t.name.toUpperCase());
        if (dist < minDist) {
            minDist = dist;
            closest = t.name;
        }
...
const createTeam = async (name, category) => {
...
const run = async () => {
    ...
    let teams = (await Team_1.default.find().select('name category')).map(t => ({ name: t.name, category: t.category }));
    let teamMap = new Map((await Team_1.default.find().select('name category _id')).map(team => [team.name.toUpperCase(), team._id]));
    let inserted = 0;
    let skipped = 0;
    for (const m of matchesData) {
        ...
        if (!homeId) {
            ...
            if (closeHome) {
                ...
                teams.push({ name: m.homeTeam, category: cat });
        ...
        if (!awayId) {
            ...
            if (closeAway) {
                ...
                teams.push({ name: m.awayTeam, category: cat });
