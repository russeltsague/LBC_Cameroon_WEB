...
function findClosestTeamInCategory(name, category, teams) {
    ...
    for (const t of candidates) {
        ...
        if (t.name.toUpperCase().replace(/ [A-Z0-9]+$/, '') === name.toUpperCase())
            return t.name;
...
const run = async () => {
    ...
    let teams = (await Team_1.default.find().select('name category _id')).map(t => ({ name: t.name, category: t.category, _id: t._id }));
    let inserted = 0;
    let skipped = 0;
    for (const c of classificationData) {
        ...
        await Classification_1.default.findOneAndUpdate({ team: teamObj._id, category: cat }, {
            team: teamObj._id,
            category: cat,
            position: c.position,
            played: c.played,
