...
class ClassificationService {
     ...
     */
    async updateClassification(matchId) {
        try {
            ...
            const homeClassification = await this.getOrCreateClassification(match.homeTeam._id, category, poule);
            const awayClassification = await this.getOrCreateClassification(match.awayTeam._id, category, poule);
     ...
     */
    async getOrCreateClassification(teamId, category, poule) {
        let classification = await Classification_1.default.findOne({
            team: teamId,
            category,
            ...(poule && { poule })
        });
        if (!classification) {
            classification = await Classification_1.default.create({
                team: teamId,
                category,
                poule,
                gamesPlayed: 0,
     ...
     */
    async isMatchAlreadyCounted(matchId, homeClassification, awayClassification) {
     ...
     */
    async calculatePositions(category, poule) {
     ...
     */
    async recalculateCategory(category, poule) {
        try {
            ...
            await Classification_1.default.updateMany(query, {
                gamesPlayed: 0,
                wins: 0,
                draws: 0,
                losses: 0,
            ...
            const matchQuery = { category, status: 'completed' };
            if (poule) {
                matchQuery.poule = poule;
            }
            const matches = await Match_1.default.find(matchQuery).populate('homeTeam awayTeam');
            ...
            return await Classification_1.default.find(query)
                .populate('team')
                .sort({ position: 1 });
        }
        catch (error) {
    ...
    async getTeamStatistics(teamId) {
        try {
            ...
            const classification = await Classification_1.default.findOne({ team: teamId });
            if (!classification) {
                return null;
            }
            // Get home and away records
            const homeMatches = await Match_1.default.find({
                homeTeam: teamId,
                status: 'completed'
            });
            const awayMatches = await Match_1.default.find({
                awayTeam: teamId,
                status: 'completed'
            });
            const homeRecord = {
...
exports.default = new ClassificationService();
