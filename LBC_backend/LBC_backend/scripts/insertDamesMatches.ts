...
async function insertDamesMatches() {
  ...
  for (const m of matchesToInsert) {
    ...
    const existing = await Match.findOne({ homeTeam: home._id, awayTeam: away._id, category: 'DAMES' });
    if (existing) {
    ...
    inserted++;
    console.log(`Inserted: ${m.homeTeam} vs ${m.awayTeam} (journee ${m.journee})`);
  }
