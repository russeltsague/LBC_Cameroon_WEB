...
export default async function EditReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addDay = () => {
    setReportData({
      ...reportData,
      matchResults: [...reportData.matchResults, {
        day: '',
  ...
  const removeDay = (dayIndex: number) => {
    setReportData({
      ...reportData,
      matchResults: reportData.matchResults.filter((_, i) => i !== dayIndex)
    })
  }

  const addMatch = (dayIndex: number) => {
    const newMatchResults = [...reportData.matchResults]
    newMatchResults[dayIndex].matches.push({ category: '', results: [''] })
    setReportData({ ...reportData, matchResults: newMatchResults })
  }

  const removeMatch = (dayIndex: number, matchIndex: number) => {
    const newMatchResults = [...reportData.matchResults]
    newMatchResults[dayIndex].matches.splice(matchIndex, 1)
    setReportData({ ...reportData, matchResults: newMatchResults })
  }

  const addResult = (dayIndex: number, matchIndex: number) => {
    const newMatchResults = [...reportData.matchResults]
    newMatchResults[dayIndex].matches[matchIndex].results.push('')
    setReportData({ ...reportData, matchResults: newMatchResults })
  }

  const removeResult = (dayIndex: number, matchIndex: number, resultIndex: number) => {
    const newMatchResults = [...reportData.matchResults]
    newMatchResults[dayIndex].matches[matchIndex].results.splice(resultIndex, 1)
    setReportData({ ...reportData, matchResults: newMatchResults })
  }
  ...
  const addSanction = () => {
    setReportData({
      ...reportData,
      sanctions: [...reportData.sanctions, {
        date: '',
        name: '',
        teamNumber: '',
        category: '',
  ...
  const removeSanction = (index: number) => {
    setReportData({
      ...reportData,
      sanctions: reportData.sanctions.filter((_, i) => i !== index)
    })
  }
  ...
  const addClassification = () => {
    setReportData({
      ...reportData,
      classifications: [...reportData.classifications, {
        title: '',
        headers: ['RANG', 'EQUIPES', 'Matchs joués', 'Victoires', 'Défaites', 'Points'],
        rows: [['', '', '', '', '', '']]
      }]
  ...
  const removeClassification = (index: number) => {
    setReportData({
      ...reportData,
      classifications: reportData.classifications.filter((_, i) => i !== index)
    })
  }

  const addClassificationRow = (classIndex: number) => {
    const newClassifications = [...reportData.classifications]
    const headers = newClassifications[classIndex].headers
    newClassifications[classIndex].rows.push(Array(headers.length).fill(''))
    setReportData({ ...reportData, classifications: newClassifications })
  }
    ...
    <AuthGuard>
      <AdminLayout>
        <div className="space-y-6">
          ...
          >
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">Modifier le Rapport</h1>
              <p className="text-gray-400">Modifiez les informations du rapport {reportData.reportTitle}</p>
            </div>
          ...
          {activeSection === 'matches' && (
            ...
            >
              ...
              {reportData.matchResults.map((day, dayIndex) => (
                <motion.div
                  key={dayIndex}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const newMatchResults = [...reportData.matchResults]
                          newMatchResults[dayIndex].day = e.target.value
                          setReportData({ ...reportData, matchResults: newMatchResults })
                        }}
                        ...
                        <div key={matchIndex} className="bg-white/5 rounded-lg p-3 border border-white/10">
                          <div className="flex items-center gap-3 mb-3">
                            <select
                              ...
                              onChange={(e) => {
                                const newMatchResults = [...reportData.matchResults]
                                newMatchResults[dayIndex].matches[matchIndex].category = e.target.value
                                setReportData({ ...reportData, matchResults: newMatchResults })
                              }}
                            ...
                            {match.results.map((result, resultIndex) => (
                              <div key={resultIndex} className="flex items-center gap-2">
                                <input
                                  ...
                                  onChange={(e) => {
                                    const newMatchResults = [...reportData.matchResults]
                                    newMatchResults[dayIndex].matches[matchIndex].results[resultIndex] = e.target.value
                                    setReportData({ ...reportData, matchResults: newMatchResults })
                                  }}
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {reportData.sanctions.map((sanction, index) => (
                        <tr key={index} className="border-b border-white/5">
                          <td className="py-3">
                            <input
                              ...
                              value={sanction.date}
                              onChange={(e) => {
                                const newSanctions = [...reportData.sanctions]
                                newSanctions[index].date = e.target.value
                                setReportData({ ...reportData, sanctions: newSanctions })
                              }}
          ...
          {activeSection === 'classifications' && (
            ...
            >
              ...
              {reportData.classifications.map((classification, classIndex) => (
                <motion.div
                  key={classIndex}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
